...
def get_latest_pruned_dir() -> pathlib.Path:
    ...
    if not base_path.exists():
        raise FileNotFoundError(f"Could not find pruned models at {base_path}")

    dirs = sorted(base_path.iterdir(), key=os.path.getmtime)
    if not dirs:
        raise FileNotFoundError(f"No pruned model directories found in {base_path}")
    return dirs[-1]
...
def run_observer(samples_per_category: int):
    ...
    cmd = [
        ...
        "--model_max_length", str(DEFAULT_MODEL_MAX_LENGTH),
...
def run_pruning(compression_ratio: float, samples_per_category: int, max_effective_params_b: float):
    import subprocess

    sync_and_show_commit()

    effective_ratio, budget = resolve_compression_ratio(compression_ratio, max_effective_params_b)
    ...
    cmd = [
        ...
        "--model-name", MODEL_NAME,
        "--profile", "false",
        "--compression-ratio", str(effective_ratio),
...
def main(
    ...
    compression_ratio: float = DEFAULT_REQUESTED_COMPRESSION_RATIO,
    ...
    run_pruning.remote(effective_ratio, samples_per_category, max_effective_params_b)
